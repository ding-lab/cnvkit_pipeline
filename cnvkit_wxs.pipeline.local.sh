...
if [[ "$FLAG" = "bed" ]]; then
  ...
  bash $SDIR/0_createGenomeBed.sh $CONFIG
  exit
fi
...
if [[ "$FLAG" = "cnn" ]]; then
  ls $bamDir/*.bam | while read bam; do
     ...
     bash $SDIR/1_call_cnn.sh $CONFIG $name $bam $OUTDIR
  done
...
if [[ "$FLAG" = "ref" ]]; then
  bash $SDIR/2_make_ref_cnn.sh $CONFIG $OUTDIR
  exit
fi
...
if [[ "$FLAG" = "cnv" ]]; then
  ls $OUTDIR/*.T.targetcoverage.cnn | while read cnn; do
    ...
    if [ -e $OUTDIR/$name.T.targetcoverage.cnn ] && [ -e $OUTDIR/$name.T.antitargetcoverage.cnn ]; then
      bash $SDIR/3_call_cnv.sh $CONFIG $PoolNorRef $name $OUTDIR
    else
      continue
    fi
  done
...
if [[ "$FLAG" = "call" ]]; then
  ls $OUTDIR/*.T.cns | while read cns; do
    fileName=${cns##*/}
    name=${fileName%.T.cns}
    bash $SDIR/call_chr1-22.sh $CONFIG $name $OUTDIR
  done
fi
...
if [[ "$FLAG" = "batch-tumor" ]]; then
	cat $TABLE | while read sample; do
		bash $SDIR/cnvkit_wxs.perTumor4poolNormal.sh -N $sample -C $CONFIG -B $bamDir/$sample.remDup.bam -R $PoolNorRef -O $OUTDIR
	done
fi
...
if [[ "$FLAG" = "callGene" ]]; then
  ls $OUTDIR/*.T.call.chr.cns | while read cns; do
    ...
    bash $SDIR/callGeneLevel_cnv.sh $CONFIG $name $OUTDIR
  done
fi
...
if [[ "$FLAG" = "scatter" ]]; then
  ls $OUTDIR/*.T.call.cns | while read cns; do
    ...
    bash $SDIR/scatter.sh $CONFIG $name $OUTDIR
  done
fi
...
if [[ "$FLAG" = "heatmap" ]]; then
  bash $SDIR/heatmap.sh $CONFIG $OUTDIR
fi
...
if [[ "$FLAG" = "batch-tn" ]]; then
    ...
    if [[ $id == "" ]];then continue; fi
		...
		bash $SDIR/cnvkit_wxs.tumor-normal.withoutPoolNor.sh -S $id -N ${bamDir}/$normal.remDup.bam -T ${bamDir}/$tumor.remDup.bam -O $OUTDIR
...
if [[ "$FLAG" = "callGene-tn" ]]; then
  ...
  do
    bash $SDIR/callGeneLevel_cnv.sh $CONFIG $sample $OUTDIR f2
  done
fi
