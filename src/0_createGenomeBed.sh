...
# make .bed
$CNVKIT access $GENOME -o $GENOME_BED

## Make splited targets bed file (default --avg-size 267)
# if target - chr start end gene
# make .split.txt
$CNVKIT target $TARGET --split -o $OUTDIR/$GENOME_TARGET
...
$CNVKIT antitarget $OUTDIR/$GENOME_TARGET --access $GENOME_BED -o $GENOME_ANTITARGET
